...
const ServiceDemandChart = () => {
  ...
  useEffect(() => {
    // Fetch bookings data
    axiosSecure.get("/booking").then((res) => {
  ...
  const calculateDynamicDemand = () => {
    ...
    bookings.forEach((booking) => {
      ...
      if (booking.category) {
        ...
        const matchingService = services.find((service) => {
          const bookingServiceName =
            booking.serviceName || booking.packageTitle || "";
          return (
            bookingServiceName
              .toLowerCase()
              .includes(service.service_name?.toLowerCase() || "") ||
            service.service_name
              ?.toLowerCase()
              .includes(bookingServiceName.toLowerCase()) ||
      ...
      if (category === "other") {
        ...
        if (serviceName.includes("wedding")) category = "wedding";
          ...
          serviceName.includes("corporate") ||
          serviceName.includes("office")
        )
          category = "corporate";
        else if (serviceName.includes("home")) category = "home";
        else if (
          serviceName.includes("seminar") ||
          serviceName.includes("meeting")
        )
          category = "seminar";
      }
    ...
    const mostPopular = serviceCategories[0] || {
      category: "N/A",
      bookings: 0,
    };
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const getServiceCategory = (booking) => {
        ...
        if (services && services.length > 0) {
          const matchingService = services.find((service) => {
            const bookingServiceName =
              booking.serviceName || booking.packageTitle || "";
            return (
              bookingServiceName.toLowerCase().includes(service.service_name?.toLowerCase() || "") ||service.service_name?.toLowerCase().includes(bookingServiceName.toLowerCase()) ||booking.serviceId === service._id);
          });
        ...
        if (serviceName.includes("corporate") || serviceName.includes("office"))
          return "corporate";
        if (serviceName.includes("home")) return "home";
        if (serviceName.includes("seminar") || serviceName.includes("meeting"))
          return "seminar";
      ...
      const monthData = {
        ...
        wedding: monthBookings.filter(
          (b) => getServiceCategory(b) === "wedding",
        ).length,
        birthday: monthBookings.filter(
          (b) => getServiceCategory(b) === "birthday",
        ).length,
        corporate: monthBookings.filter((b) =>
          ["corporate", "office"].includes(getServiceCategory(b)),
        ).length,
        home: monthBookings.filter((b) => getServiceCategory(b) === "home")
          .length,
        seminar: monthBookings.filter((b) =>
          ["seminar", "meeting"].includes(getServiceCategory(b)),
        ).length,
      };
    ...
    const calculateTopServices = () => {
      if (!services || services.length === 0) {
        ...
        bookings.forEach((booking) => {
          const serviceName =
            booking.serviceName || booking.packageTitle || "Unknown Service";
      ...
      const serviceStats = services.map((service) => {
        ...
        const previousSixMonths = bookings.filter((booking) => {
          ...
          return (
            isMatchingService &&
            bookingDate >= twelveMonthsAgo &&
            bookingDate < sixMonthsAgo
          );
        }).length;
    ...
    const lastSixMonthsTotal = monthlyDemand
      .slice(-6)
      .reduce(
        (sum, m) =>
          sum + m.wedding + m.birthday + m.corporate + m.home + m.seminar,
        0,
      );
    const previousSixMonthsTotal = monthlyDemand
      .slice(-12, -6)
      .reduce(
        (sum, m) =>
          sum + m.wedding + m.birthday + m.corporate + m.home + m.seminar,
    ...
    const growthRate =
      previousSixMonthsTotal > 0
        ? Math.round(
            ((lastSixMonthsTotal - previousSixMonthsTotal) /
              previousSixMonthsTotal) *
              100,
          )
        : 0;
  ...
  const maxBookings =
    dynamicDemand.monthlyDemand.length > 0
      ? Math.max(
          ...dynamicDemand.monthlyDemand.map(
            (month) =>
              month.wedding +
              month.birthday +
              month.corporate +
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div
          className="stat bg-white shadow-xl rounded-lg border-l-4"
          style={{ borderLeftColor: "#628141" }}
        >
        ...
        >
          ...
          >
            {dynamicDemand.growthRate >= 0 ? "+" : ""}
            {dynamicDemand.growthRate}%
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card bg-base-100 shadow-xl">
          <div className="card-body">
            ...
            <div className="space-y-4">
              {dynamicDemand.topServices.length > 0 ? (
                dynamicDemand.topServices.map((service, index) => (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <div className="text-right">
                      <span
                        ...
                        style={{
                          backgroundColor: service.trend.startsWith("+")
                            ? "#628141"
                            : "#dc2626",
                          borderColor: service.trend.startsWith("+")
                            ? "#628141"
                            : "#dc2626",
                        }}
                      >
                ...
                <div className="text-center text-gray-500 py-8">
                  ...
                  <p className="text-sm">
                    Services will appear here once booking data is linked to
                    service records
                  </p>
                </div>
              )}
      ...
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body">
          <h2 className="card-title text-2xl mb-6">
            Monthly Service Demand (Histogram)
          </h2>
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              <div
                className="w-4 h-4 rounded"
                style={{ backgroundColor: "#628141b3" }}
              ></div>
            ...
            <div className="flex items-center gap-2">
              <div
                className="w-4 h-4 rounded"
                style={{ backgroundColor: "#1B211Ab3" }}
              ></div>
